...
/**
 * @summary Search for an image and return a URL
 */
export const getAiImageSearchUrl = () => {
  return `/api/ai/image-search`;
};

export const aiImageSearch = async (
  aiImageSearchRequest: AiImageSearchRequest,
  options?: RequestInit,
): Promise<AiImageSearchResponse> => {
  return customFetch<AiImageSearchResponse>(getAiImageSearchUrl(), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(aiImageSearchRequest),
  });
};

export const getAiImageSearchMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof aiImageSearch>>,
    TError,
    { data: BodyType<AiImageSearchRequest> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  ...
  TContext
> => {
  const mutationKey = ["aiImageSearch"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
  ...
  > = (props) => {
    ...
    return aiImageSearch(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type AiImageSearchMutationResult = NonNullable<
  Awaited<ReturnType<typeof aiImageSearch>>
>;
export type AiImageSearchMutationBody = BodyType<AiImageSearchRequest>;
export type AiImageSearchMutationError = ErrorType<unknown>;
...
>(options?: {
  ...
  TContext
> => {
  return useMutation(getAiImageSearchMutationOptions(options));
};
